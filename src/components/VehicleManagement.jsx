...
const VehicleManagement = () => {
    ...
    const fetchVehicles = async () => {
        try {
            ...
            console.log("Vehicles data:", response.data.results);
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        const { license_plate, model, capacity, source, rental_company, driver, status, fuel_type, fuel_efficiency, total_km } = newVehicle;

        // Validation
        if (!license_plate || !model || !capacity || !source || !driver || !status || !fuel_type || !fuel_efficiency || !total_km) {
        ...
        const vehicleData = {
            ...
            total_km: Number(total_km)  // Include total_km in the payload
        };

        try {
            if (editingVehicle) {
                await axios.put(ENDPOINTS.EDIT_VEHICLE(editingVehicle.id), vehicleData, {
                    headers: { Authorization: `Bearer ${token}` },
                });
                toast.success("Vehicle updated successfully!");
            } else {
                await axios.post(ENDPOINTS.VEHICLE_LIST, vehicleData, {
                    headers: { Authorization: `Bearer ${token}` },
                });
    ...
    const handleEdit = (vehicle) => {
        ...
        setNewVehicle({
            ...
            total_km: vehicle.total_km || ""  // Set total_km when editing
        });
        setShowModal(true);
    };
                                    ...
                                    <div className="row mb-3">
                                        <div className="col-md-6">
                                            ...
                                            <input
                                                type="number"
                                                className="form-control"
                                                value={newVehicle.total_km || ""}
                                                onChange={(e) => setNewVehicle({ ...newVehicle, total_km: e.target.value })}
